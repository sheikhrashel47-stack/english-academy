...
import { recommendMasterStartingLesson, scoreAssessment, selectAssessmentQuestions, type SelectedAssessmentQuestion } from "@/domain/learning/assessmentEngine";
...
export function AssessmentResultPanel({ blueprint, result }: { blueprint: AssessmentBlueprint; result: AssessmentResult }) {
  const startLesson = result.recommendedStartingLessonId;
  return <section className="assessment-result"><div className={result.passed ? "result-medallion passed" : "result-medallion"}>{result.passed ? <CheckCircle2 /> : <TimerReset />}<span>{result.score}%</span></div><div><span className="eyebrow">Assessment result</span><h2>{result.passed ? "ভিত্তি শক্ত হয়েছে" : "এখান থেকেই recovery শুরু"}</h2><p>{result.passed ? "এই educational result স্থানীয় learning record-এ রাখা হয়েছে।" : "এটি official certification নয়; ভুলগুলো দেখে আবার অনুশীলন করো।"}</p><div className="result-stat-row"><span>{result.earnedPoints}/{result.totalPoints} points</span><span>{result.manualReviewQuestionIds.length} manual review</span><span>{result.wrongQuestionIds.length} review items</span></div>{startLesson && <div className="placement-recommendation"><span className="card-kicker">তোমার recommended starting point</span><strong>{result.recommendedStartingLessonTitle}</strong><p>{result.placementNote} এটি official CEFR determination নয়; তোমার learning journey শুরু করার একটি evidence-based recommendation।</p><Button type="button" onClick={() => { window.location.href = `/lesson/${startLesson}`; }}>এই lesson থেকে শুরু করো</Button></div>}</div><div className="result-sections">{result.sectionScores.map((section) => <div key={section.sectionId}><strong>{section.skill}</strong><span>{section.score}%</span></div>)}</div></section>;
}
