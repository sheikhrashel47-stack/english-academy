...
/* Emerald Study House — vocabulary is an English-first study library with Bangla guidance, a CEFR ledger and restrained Emerald status cues. */
...
export default function VocabularyPage() {
  ...
  const bootstrapPending = Boolean(result && !query && level === "all" && topic === "all" && state === "all" && page === 0 && result.total < 20_000);
  useEffect(() => {
    if (!bootstrapPending) return;
    const retry = window.setTimeout(() => {
      void learningUseCases.searchVocabulary({ page: 0, pageSize: 18 }).then(setResult);
    }, 1500);
  ...
  return <AppShell eyebrow="Vocabulary" title="Vocabulary study desk">
    <section className="vocabulary-intro paper-card"><div><p className="card-kicker"><BookOpenCheck size={14} /> Vocabulary study library</p><h2>Find a word.<br /><em>Use it with confidence.</em></h2><p>বাংলা সহায়তায় শব্দের অর্থ, উচ্চারণ ও ব্যবহার দেখো। একসঙ্গে ছোট একটি page দেখানো হয়—তাই ২০,০০০+ record-এও search দ্রুত থাকে।</p><div className="vocabulary-ledger" aria-label="Vocabulary corpus study ledger"><span><b>CEFR</b> Pre-A1–C2</span><span><b>Catalogue</b> {bootstrapPending ? "২০,০০০+ প্রস্তুত হচ্ছে" : `${result?.total ?? "…"} verified words`}</span><span><b>Mode</b> First load-এর পরে offline</span></div></div><div className="vocabulary-count"><strong>{bootstrapPending ? "…" : result?.total ?? "—"}</strong><span>{bootstrapPending ? "offline library প্রস্তুত হচ্ছে" : "matching words"}</span></div></section>
