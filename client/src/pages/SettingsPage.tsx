 ...
 */
import { Check, Database, Download, Languages, Moon, Palette, RotateCcw, Smartphone, Sun, Target, Upload, Volume2, WandSparkles } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [prefs, setPrefs] = useState({ soundEnabled: true, hapticEnabled: false, animationsEnabled: true, reducedMotion: false, dailyGoalMinutes: 15 as 10 | 15 | 20 | 30 });
  const fileInput = useRef<HTMLInputElement>(null);
  const selected = themes.find((item) => item.id === theme) ?? themes[0];
  useEffect(() => {
    if (settings) setPrefs({ soundEnabled: settings.soundEnabled, hapticEnabled: settings.hapticEnabled ?? false, animationsEnabled: settings.animationsEnabled, reducedMotion: settings.reducedMotion, dailyGoalMinutes: settings.dailyGoalMinutes });
  }, [settings]);
