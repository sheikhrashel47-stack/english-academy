...
export default function FlashcardsPage() {
  ...
  const rate = async (rating: typeof ratings[number]) => { if (!entry) return; await learningUseCases.recordFlashcardReview(entry.item.id, rating); toast.success("Review locally saved হয়েছে।"); setFlipped(false); setIndex((current) => current + 1 >= entries.length ? 0 : current + 1); load(); };
