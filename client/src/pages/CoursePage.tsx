...
export default function CoursePage() {
  ...
  const selectedCourseId = routeCourseId ?? "course-english-foundations";
  const isMasterCourse = selectedCourseId === "course-english-master";
  const courseTitle = isMasterCourse ? "Master English Journey" : "English Foundations";
  const currentLevelCode = isMasterCourse ? "Pre-A1" : "A1";
  const [items, setItems] = useState<RoadmapItem[]>([]);
  const [courseMap, setCourseMap] = useState<CourseMap>([]);
  useEffect(() => { void Promise.all([learningUseCases.getRoadmap(selectedCourseId), learningUseCases.getCourseMap(selectedCourseId)]).then(([roadmap, map]) => { setItems(roadmap); setCourseMap(map); }); }, [selectedCourseId]);
  const completed = items.filter((item) => item.progress?.completed).length;
  const ribbonLevels = courseMap.filter(({ level }, index, entries) => index === entries.findIndex((entry) => entry.level.code === level.code));
  return <AppShell eyebrow="Learning map" title={courseTitle}>
    <section className="academy-course-banner paper-card"><div><p className="card-kicker">CEFR-aligned learning path</p><h2>শুনে, বলে ও গড়ে<br /><em>প্রতিদিনের ইংরেজি।</em></h2><p>এই course-এ {items.length}টি interactive lesson, grammar practice এবং vocabulary review রয়েছে।</p></div><div className="cefr-ribbon" aria-label="Pre-A1 থেকে C2 CEFR mastery ribbon"><span className="cefr-ribbon-label">Your CEFR journey</span>{ribbonLevels.map(({ level }) => <div className={`cefr-segment ${level.code === currentLevelCode ? "cefr-segment-current" : level.availability === "coming-soon" ? "cefr-segment-future" : ""}`} key={level.id}><strong>{level.code}</strong><small>{level.code === currentLevelCode ? "Current" : level.availability === "coming-soon" ? "Next" : "Open"}</small></div>)}</div></section>
